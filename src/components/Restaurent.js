  ...
  return (
    <div className=" w-6/12 mx-auto items-center justify-center flex flex-col">
      <div id="Resturent details " className="w-full flex justify-between ">
          ...
          <h5 className=" my-6 bg-green-700 text-white p-2 m-2 rounded-full"> ✦ {avgRatingString}</h5>
      </div>
      <h1 className=" text-green-800 p-2 m-2 text-2xl">{"୧‿̩͙ ˖︵ Menu ︵˖ ‿̩͙୨"}</h1>
      {menuItems.map((cat)=>{
        if(cat.card.card["@type"] === "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory"){
          return <MenuCategory key={cat.card.card.title} data={cat} />;
        }
      })}
    </div>
  );
